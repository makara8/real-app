...
const messageCtrl = {
    createMessage: async (req, res) => {
        try {
            const { sender, recipient, text, media, call,voice,edit } = req.body
            ...
            const newConversation = await Conversations.findOneAndUpdate({
                ...
                recipients: [sender, recipient],
                text, media, call,voice
            }, { new: true, upsert: true })

            const newMessage = new Messages({
                conversation: newConversation._id,
                sender, call,
                recipient, text, media,voice,edit
            })

            await newMessage.save()

            res.json({msg: 'Create Success!',_id:newMessage._id})
    ...
    EditMessage: async (req,res) => {
        const {media,text,edit} = req.body
        const message = await Messages.findByIdAndUpdate(req.params.id,{
            media,
            text,
            edit
        })
